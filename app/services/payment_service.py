...
async def create_stripe_checkout_session(
    ...
    session = stripe.checkout.Session.create(
        ...
        success_url=f"{settings.PAYMENT_CONFIRMATION_URL}?session_id={{CHECKOUT_SESSION_ID}}",
...
async def fullfil_order_payment(db: AsyncSession, session_id: str) -> Order:
    ...
    try:
        ...
        raise ApiException(
            f"Stripe session error: {str(e)}", status.HTTP_400_BAD_REQUEST
        )
    ...
    if not order:
        raise ApiException(
            f"Order #{order_id} not found in our records.", status.HTTP_404_NOT_FOUND
        )
    ...
    if session.payment_status == "paid":
        ...
        # Confirmed successful order receipt notification
        ...
        await notify_delivery_team_of_order(order, order.user.name)
        ...
        raise ApiException(
            "Payment was not successful. Please try again.", status.HTTP_400_BAD_REQUEST
        )


async def cancel_order_payment(db: AsyncSession, session_id: str) -> Order:
    # 1. Retrieve session from Stripe
    try:
        session = stripe.checkout.Session.retrieve(
            session_id, api_key=settings.STRIPE_SECRET_KEY
    ...
    if not order:
        raise ApiException(f"Order #{order_id} not found.", status.HTTP_404_NOT_FOUND)
