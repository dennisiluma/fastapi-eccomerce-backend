...
async def fetch_or_create_cart_model(db: AsyncSession, user_id: int) -> Cart:
    statement = (
        select(Cart)
    ...
    if not cart:
        ...
        return await fetch_or_create_cart_model(db, user_id)
...
async def add_item_to_cart(
    ...
    cart = await fetch_or_create_cart_model(db, user_id)
    ...
    cart_item = await db.scalar(item_statement)
...
async def decrement_item_quantity(
    ...
    cart_item = await db.scalar(statement)
...
async def clear_user_cart(db: AsyncSession, user_id: int) -> bool:
    cart = await fetch_or_create_cart_model(db, user_id)
...
async def get_user_cart_summary(db: AsyncSession, user_id: int) -> dict:
    cart = await fetch_or_create_cart_model(db, user_id)
