...
async def view_cart(
    db: AsyncSession = Depends(db_session), user: User = Depends(get_current_user)
) -> ApiResponse[CartRead]:

    cart = await get_user_cart_summary(db, user.id)
...
async def add_to_cart(
    ...
    return ApiResponse[None](status=status.HTTP_200_OK, message="Item added to cart")
...
async def remove_from_cart(
    ...
    return ApiResponse[None](status=status.HTTP_200_OK, message="Item removed")
